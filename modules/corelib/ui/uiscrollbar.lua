...
local function updateSlider(self)
  local slider = self:getChildById('sliderButton')
  --local sliderHor = self:getChildById('sliderButtonHor')
  if slider == nil then return end

  local range, pxrange, px, offset, center = calcValues(self)

  if self.orientation == 'vertical' then
    if(self.orientation == 'vertical' and px <= 9) then
      px = 8
      slider:setImageSource('/images/ui/scrollbars/sliderVerSmall')
      slider:setImageBorder(6)
      slider:setImageClip(torect("0 0 17 13"))
    else
      slider:setImageSource('/images/ui/scrollbars/sliderVer')
      slider:setImageBorder(9)
      slider:setImageClip(torect("0 0 17 38"))
    end
    slider:setHeight(px)
    slider:setMarginTop(offset)
  else -- horizontal
    if(self.orientation == 'horizontal' and px <= 9) then
      px = 8
      slider:setImageSource('/images/ui/scrollbars/sliderHorSmall')
      slider:setImageBorder(6)
      slider:setImageClip(torect("0 0 13 17"))
    else
      slider:setImageSource('/images/ui/scrollbars/sliderHor')
      slider:setImageBorder(9)
      slider:setImageClip(torect("0 0 38 17"))
    end
    slider:setWidth(px)

...
function offline()
  ...
  -- Garantir que não há mouse grabbed
...
function online()
  ...
  -- Garantir que estados anteriores estejam limpos
  if g_ui.isMouseGrabbed() then
    mouseGrabberWidget:ungrabMouse()
  end
  g_mouse.popCursor('target')
...
function assignSpell(widget)
  ...
  -- hide the checkPanel since we no longer need it
  window.checkPanel:setVisible(false)

  local spells = modules.gamelib.SpellInfo['Default']
  local vocation = g_game.getLocalPlayer():getVocation()

  for spellName, spellData in pairs(spells) do
    if table.find(spellData.vocations, vocation) then
  ...
  -- Remove the filterByVocation function and related callbacks
  -- as we no longer need to filter after loading
...
function onDragStart(self)
  ...
  -- Desabilita temporariamente o scroll

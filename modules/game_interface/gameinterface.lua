...
function onLoginAdvice(message)
  displayInfoBox(tr("Informa��o"), message)
end
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if creatureThing then
    ...
    if creatureThing:isLocalPlayer() then
      ...
      menu:addOption(tr('Inspect'), function() modules.game_inspect.inspect(creatureThing) end)

    else
      if creatureThing:isPlayer() then
        local creatureName = creatureThing:getName()
        
        menu:addOption(tr('Look'), function() g_game.look(creatureThing) end)
        menu:addOption(tr('Inspect'), function() modules.game_inspect.inspect(creatureThing) end)

        if g_game.getFeature(GameBrowseField) then
          menu:addOption(tr('Browse Field'), function() g_game.browseField(creatureThing:getPosition()) end)
        end
        ...
        if localPlayerShield == ShieldNone or localPlayerShield == ShieldWhiteBlue then
          if creatureShield == ShieldWhiteYellow then
            ...
            menu:addOption(tr('Invite to Party'), 
              function() g_game.partyInvite(creatureThing:getId()) end)
          end
        end
...
local function getOwnPrivateTab()
  if not modules.game_console or not modules.game_console.getOwnPrivateTab then
    return nil
  end
  return modules.game_console.getOwnPrivateTab()
end

local privateTab = getOwnPrivateTab()
...
function addVip()
  local name = addVipWindow:getChildById('name'):getText()
  local localPlayer = g_game.getLocalPlayer()
  
  -- Verifica se está tentando adicionar a si mesmo
  if name:lower() == localPlayer:getName():lower() then
    modules.game_textmessage.displayStatusMessage(tr("You cannot add yourself to VIP list."))
    destroyAddWindow()
    return
  end
...
function onVipListLabelMousePress(widget, mousePos, mouseButton)
  ...
  local name = widget:getText()
  local id = widget:getId():sub(4) -- Remove 'vip' do início do ID

  menu:addOption(tr('Send Message'), function() g_game.openPrivateChannel(name) end)
  menu:addOption(tr('Remove %s', name), function() removeVip(widget) end)
  menu:addSeparator()
...
function createVipMenu(mousePos, vipId)
  local menu = g_ui.createWidget('PopupMenu')
  local vipInfo = VipList[vipId]
  
  if not vipInfo then return end

  menu:addOption(tr('Message'), function() g_game.openPrivateChannel(vipInfo.name) end)
  
  if modules.game_console then
    if modules.game_console.isIgnored(vipInfo.name) then
      menu:addOption(tr('Unignore') .. ' ' .. vipInfo.name, function() modules.game_console.removeIgnoredPlayer(vipInfo.name) end)
    else
      menu:addOption(tr('Ignore') .. ' ' .. vipInfo.name, function() modules.game_console.addIgnoredPlayer(vipInfo.name) end)
    end
  end

  -- Adiciona opção para remover da VIP list
  menu:addOption(tr('Remove'), function() g_game.removeVip(vipId) end)
  
  menu:addSeparator()
  menu:addOption(tr('Copy Name'), function() g_window.setClipboardText(vipInfo.name) end)

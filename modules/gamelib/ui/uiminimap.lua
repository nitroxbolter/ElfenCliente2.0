...
function UIMinimap:fragCreature(widget, pos, desc, time)
  if not time then time = 1000 end
  if not pos or not widget then return end
  widget.pos = pos
  widget.description = desc or ''
  widget:setTooltip(desc)
  self:centerInPosition(widget, widget.pos)
  scheduleEvent(function() self:removeChild(widget) end, time)
end